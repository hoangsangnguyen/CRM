    ...
    $('#btn_search').click(function () {
        ...
        var dataSource = new kendo.data.DataSource({
            transport: {
                prefix: "",
                read: {
                    url: "/admin/tenant/search/?keyword=" + keyword,
                    dataType: "json",
                    type: "POST"
                }
            },
...
function gridInit() {
    ...
    var dataSource = new kendo.data.DataSource({
        transport: {
            ...
            update: { url: "/admin/tenant/edit" },
            destroy: { url: "/admin/tenant/DeleteConfirmed" }
        },
        pageSize: 10,
        page: 1,
        sortable: true,
...
function gridConfigInit(dataSource) {
    ...
    jQuery("#grid").kendoGrid({
        ...
        //},
        {
            "title": "TenantName", "attributes": { "id": "TenantName", "name": "TenantName" },
            "template": "<span class='grid-row-edit' data-id='#:TenantId#'>#:TenantName#</span>",
            "field": "TenantName", "filterable": {}, "encoded": true, "editor": null,
            sortable: true,
        },
         {
             "title": "TenantNameAlias", "attributes": { "id": "TenantNameAlias", "name": "TenantNameAlias", "class": "hidden-1090" },
             "field": "TenantNameAlias", "filterable": {}, "encoded": true, "editor": null,
         ...
         {
             "title": "CompanyLogo", "attributes": { "id": "CompanyLogo", "name": "CompanyLogo", "class": "hidden-610" },
             "field": "CompanyLogo", "filterable": {}, "encoded": true, "editor": null,
             "template": "<img alt=\"\" src=\"#= CompanyLogo # \">",
             sortable: false,
         },
         {
             "title": "OrgNumber", "attributes": { "id": "OrgNumber", "name": "OrgNumber", "class": "hidden-410" },
             "field": "OrgNumber", "filterable": {}, "encoded": true, "editor": null,
             sortable: false,
         },
         {
         ...
         },
         {
             "template": "<div class=\"hidden-sm hidden-xs btn-group\">"
                 + "<button class=\"btn_table\" onClick=\"detailTenant(#= TenantId #)\" ><i class=\"multi-icon fa icon_menu-square_alt2 bigger-120\"></i></button>"
                 + "<button class=\"btn_table\" onClick=\"editTenant(#= TenantId #)\" ><i class=\"multi-icon fa icon_pencil bigger-120\"></i></button>"
                 + "<button class=\"btn_table\" onClick=\"deleteTenant(#= Active #, #= TenantId #)\" ><i class=\"multi-icon fa icon_trash_alt red bigger-120\"></i></button>"
                 ...
                 + "<li><a title data-rel=\"tooltip\" class=\"tooltip-info\" onClick=\"detailTenant(#= TenantId #)\" data-original-title=\"View\"><i class=\"multi-icon fa icon_menu-square_alt2 bigger-120\"></i></a> </li>"
                 + "<li><a title data-rel=\"tooltip\" class=\"tooltip-success\" onClick=\"editTenant(#= TenantId #)\"  data-original-title=\"Edit\"><i class=\"multi-icon fa icon_pencil bigger-120\"></i></a> </li>"
                 + "<li><a title data-rel=\"tooltip\" class=\"tooltip-error\" onClick=\"deleteTenant(#= Active #, #= TenantId #)\"  data-original-title=\"Delete\"><i class=\"multi-icon fa icon_trash_alt red bigger-120\"></i></a> </li>"
                 + "</ul></div></div> </td>",
             "field": null,
             "sortable": false,
             "filterable": false,
...
function detailTenant(id) {
    ...
    var siteUrl = "/admin/tenant/details/";
    $(location).attr('href', siteUrl + id);
}

function editTenant(id) {
    var siteUrl = "/admin/tenant/edit/";
    $(location).attr('href', siteUrl + id);
}

function deleteTenant(active, idDelete) {
    var titleMessage = "";
    var siteUrl = "";
    if (active == true) {
        siteUrl = "/admin/tenant/deactive";
        titleMessage = "Do you want to deactive this tenant?";
    }
    else {
        siteUrl = "/admin/tenant/active";
        titleMessage = "Do you want to active this tenant?";
    }
    ...
    //activeTenant(idDelete, siteUrl, divConfirm);
}

function activeTenant(tenantId, siteUrl, divConfirm) {
    $('.show_massage').hide();
...
function EditTenant123(e) { // before EditTenant(e) {
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    var id = dataItem.TenantId;
    var siteUrl = "/admin/tenant/edit/";
...
function confirmDelete123(e) {
    $('.show_massage').hide();
    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    var idDelete = dataItem.TenantId;
    var siteUrl = "/admin/tenant/delete";
...
function DeleteFieldChoose() {
    $(".js-btn-delete-selected").unbind("click").click(function () {
        var siteUrl = "/admin/tenant/DeleteRecords";
        var dialog = new CRMDialog("modal-mask");
        var kendoWindow = dialog.showDialog($("#confirmDialogTitle").val(), $("#delete-confirmation").html());
        kendoWindow
            .find(".delete-confirm,.delete-cancel")
...
function AddEventListener(e) {
    $('.js-add-category').click(function () {
        ...
        if (siteUrl != null) {
            $(location).attr('href', siteUrl + '/tenant/add/');
        } else {
            $(location).attr('href', '/tenant/add/');
        }
    });
}
